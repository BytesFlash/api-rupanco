...
function procesarJSON(data, requerimiento, procesarResultado)
{
	...
	var async = false; // llamada sincrona FALSE, asincrona TRUE
	x.open('POST','https://plugin.autentia.mb:7777/'+requerimiento, async);
	...
	x.onreadystatechange = function(){
		...
		if (isValid) {
			if (typeof data.ParamsGet.error === "undefined") {
				//
			} else {
				alert(data.ParamsGet.error);
			}
			return fn(data);
		} else {
...
function AutentiaMobile()
{
	debugLog('Plugin Android Loaded!!');
};
...
function AutentiaFactory()
{
	...
	if (isAndroid) {
		return new AutentiaMobile();
	}
	else {
		return new plgAutentiaJS();
	};
...
plgAutentiaJS.prototype.OperacionAutentia = function(operacion, rutVerif, excResp) 
{
	switch (operacion) {
		case "VerificaHuellaStandard":
			...
			this.Transaccion2('../FEDAUTENTIA/verificadatos', entrada, salida, true, token, function(response) {
				excResp(response);
			});
			break;
		case "VerificaHuellaDEC4":
			...
			var token = Math.random();
			var salida = ['Erc', 'ErcDesc', 'NroAudit'];
			this.Transaccion2('../DEC4/verificaDEC', entrada, salida, true, token, function(response) {
				excResp(response);
			});
			break;
		default:
...
AutentiaMobile.prototype.OperacionAutentia = function(operacion, rutVerif, excResp) 
{	
	response= {erc:-2, ercText:"MODULO NO IMPLEMENTADO"}
	switch (operacion) {
		case "VerificaHuella":
			excResp(response);
		default:
