...
function Verificar(rut, excResp)
{   
    var respuesta;
	Ttoken = Math.random().toString();  
	try {
		...
		var salidas = ["ErcDesc", "ErcCod", "Nombres", "ApPaterno", "ApMaterno", "Vigencia", "NroAudit", "Restriccion"];
		...
		Autentia.Transaccion2("../ENTELPERU/verifica_telco", entradas, salidas, focoAutentia, Ttoken, function(resultado){
            excResp(resultado);
		});
	} catch (ex) {
        respuesta.ErcCod = -666;
		respuesta.ErcDesc = ex.message;
        excResp(respuesta);
	}
