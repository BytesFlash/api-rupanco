...
//
//  PAM 20210323: 
//  - se agrega verificación del token original
//  - se mejoran condiciones para rechazar ataques del tipo MiTM
//  - se agrega función que genera Token para evitar problemas con decimales de Math.random
//  - la respuesta incluye los elementos MBerc, MBercText y MBversion
*/
	...
	try {
		var ajaxResult = jQuery.ajax ({
			...
			success: function(encodedResp) {
				...
				// PAM 20210323 - se verifica que el token de la respuesta sea el enviado
				if ( data.token != resp.token ) {
					resp = {
						MBerc: -100,
						MBercText: 'El token de la respuesta no calza con el enviado. ' + resp.MBercText,
						MBversion: resp.MBversion
					};
				} else if( exitoAutentia(resp) ) {
					if (FirmaInvalidaMB(resp, encodedResp)) {
						resp = { // PAM 20210323 - se modifica el codigo de error en caso que no calce la firma
							MBerc: -101,
							MBercText: 'La firma de la respuesta es inválida. ' + resp.MBercText,
							MBversion: resp.MBversion
						};
					}
				}
			...
			error: function() {
				procesarResultado({
					MBerc: -102, // PAM 20210323 - se asigna código de error
					MBercText: 'Error de comunicacion con la componente Autentia.'
				});
			},
			complete: function() {},
			dataType: 'text',
...
//			procesarResultado({ParamsGet: {erc: -1, ercText: 'AjaxError: ' + ajaxResult.statusText}});
			procesarResultado({MBerc: -103, MBercText: 'AjaxError: ' + ajaxResult.statusText});
		}
	} catch (e){
		console.log("ERROR", e);
//		procesarResultado({ParamsGet: {erc: -1, ercText: 'ERROR: ' + e.message}});
		procesarResultado({MBerc: -104, MBercText: 'ERROR: ' + e.message});
	}
} // procesarJSON
...
//    return parseInt(valueOrDefault(function() {return respuesta.ParamsGet.erc;})) === 0;
    return parseInt(valueOrDefault(function() {return respuesta.MBerc;})) === 0;
}
...
// PAM 20210323 - Se genera el token para que sea del tipo que se necesita y no perder decimales
plgAutentiaJS.prototype.GenToken = function() {
...
// Funcion que permite llamar a una transacción de Autentia
// La respuesta incluye los elementos MBerc, MBercText con el resultado de la comunicación o llamada, 
// que es independiente del resultado de la transacción. El resultado de la trasacción sólo el válido si MBerc == 0
// La respuesta tambien incluye MBversion con la versión del MultiBrowser
