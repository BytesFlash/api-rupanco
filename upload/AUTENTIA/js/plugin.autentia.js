...
function procesarJSON(data, requerimiento, procesarResultado)
{
	...
	if(window.XDomainRequest){
		...
		xdr.onerror = function () {
			alert ('Error al comunicar con multibrowser...'); };
		xdr.onload = function() {
			data = JSON.parse(xdr.responseText);

			//var tmpSignature = JSON.stringify(data).replace(/"signature":"[^"]+"/, '"signature":""');
			var tmpSignature = xdr.responseText.replace(/"signature":"[^"]+"/, '"signature":""');
		...
		xdr.open('POST','http://plugin.autentia.mb:7777/'+requerimiento+'?ssl', async);
		setTimeout(function () {xdr.send(string);}, 0);
	} else {
