            ...
            success: function(encodedResp) {
                ...
                if (exitoAutentia(resp)) {
                    ...
                    if (! firmaValida) {
                        ...
                        tmpSignature = encodedResp.replace(/"signature":"[^"]+"/, '"signature":""');
                        tmpSignature = tmpSignature.replace(/"token":"[^"]+"/, '"token":""');
                        firmaValida = x509.subjectPublicKeyRSA.verifyString(tmpSignature, resp.signature);
                        if (! firmaValida) {
                        resp = {
                            ParamsGet: {
...
ProcesoFirmaPDF2.prototype.solicitarFirmaDocumento = function() {
    if (this.certificadoSeleccionado === null) {
        ...
        $.blockUI(mensajeBloqueo("Seleccionando el certificado del firmante...", true));
